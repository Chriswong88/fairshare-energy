...
import {createBuyerBillsStatementPdf} from '@/lib/pdf-statement';
...
export async function GET(request: NextRequest) {
  ...
  const summary = buildBuyerBillsSummary(transactions);
  const planInfo = getElectricityPlanInfo(profileResult.data?.electricity_provider, profileResult.data?.electricity_plan);
  const pdf = createBuyerBillsStatementPdf({summary, planInfo, profile: profileResult.data});
  const filename = `fairshare-buyer-statement-${new Date().toISOString().slice(0, 10)}.pdf`;
